...
static int redislite_keys_are_equal(redislite_page_index_key* key1, redislite_page_index_key* key2) {
	if (key1->keyname_size != key2->keyname_size) return 0;
		...
		for (i=0; i < page->number_of_keys; i++) {
			...
			if (cmp_result == 0) {
				if (page->keys[i]->keyname_size == length) {
					...
					if (page_num) *page_num = _page_num;
					return ret;
				}
...
int redislite_delete_keys(void *_cs, int q, char **keys, size_t* lengths)
{
	...
	for (i = 0; i < q; i++) {
		...
		if (status != REDISLITE_OK && status != REDISLITE_NOT_FOUND) break;
		if (status == REDISLITE_OK) counter++;
	}

	if (status == REDISLITE_OK) status = counter;
	return status;
}
